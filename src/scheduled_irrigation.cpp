...
void ScheduledIrrigation::setup(int zoneIndex)
{
    ...
    _scheduleCount = 0;
    _lastTriggerTime = 0;
    Trace::log(TraceLevel::INFO, "ScheduledIrrigation initialized for zone " + String(zoneIndex));
}
...
void ScheduledIrrigation::loop(unsigned long currentTime)
{
    ...
    {
        if (_schedules[i].enabled && isScheduleTimeMatch(_schedules[i], currentTime))
        {
            // Check if enough time has passed since last trigger (prevent retriggering in same minute)
            if ((currentTime - _lastTriggerTime) > 60000)  // More than 1 minute
            {
                _lastTriggerTime = currentTime;
                Trace::log(TraceLevel::INFO, "Schedule triggered for zone " + String(_zoneIndex) + 
                          " - running for " + String(_schedules[i].durationMinutes) + " minutes");
                // Trigger action would be handled by caller (e.g., start irrigation zone)
            }
        }
    }
}
...
void ScheduledIrrigation::addSchedule(const IrrigationSchedule& schedule)
{
    if (_scheduleCount < MAX_SCHEDULES_PER_ZONE)
    {
        ...
        _scheduleCount++;
        Trace::log(TraceLevel::INFO, "Schedule added for zone " + String(_zoneIndex));
    }
    else
    {
        Trace::log(TraceLevel::ERROR, "Cannot add schedule - max schedules reached for zone " + String(_zoneIndex));
    }
}
...
void ScheduledIrrigation::removeSchedule(int scheduleIndex)
{
    if (scheduleIndex >= 0 && scheduleIndex < _scheduleCount)
    {
        ...
        _scheduleCount--;
        Trace::log(TraceLevel::INFO, "Schedule removed from zone " + String(_zoneIndex));
    }
}

void ScheduledIrrigation::updateSchedule(int scheduleIndex, const IrrigationSchedule& schedule)
{
    if (scheduleIndex >= 0 && scheduleIndex < _scheduleCount)
    {
        _schedules[scheduleIndex] = schedule;
        Trace::log(TraceLevel::INFO, "Schedule updated for zone " + String(_zoneIndex));
    }
}
...
bool ScheduledIrrigation::isScheduleTimeMatch(const IrrigationSchedule& schedule, unsigned long currentTime) const
{
